...
import {NODE_TYPE_NAMES, NodeType, type Solid, UNKNOWN} from '@solid-devtools/debugger/types'
...
export function paddedForEach<T, I extends string>(
    ...
    mapped.forEach(([item, paddedValue], index) =>
        callback(getPaddedText(paddedValue), item, index),
    )
}
