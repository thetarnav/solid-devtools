...
export function OwnerNode(props: { owner: DeepReadonly<MappedOwner> }): JSX.Element {
	...
	return (
		<div
			ref={ref}
			class="bg-cyan-200/10 border-0 border-t-1px border-l-1px border-cyan-900/30 outline-1px text-12px pt-1 pl-0.5"
		>
			<div class="pb-1 pr-2">
				<p class="italic pb-0.5">
					{type === OwnerType.Component ? `<${name}>` : name}{" "}
					<span class="text-10px opacity-40">{typeName}</span>
				</p>
