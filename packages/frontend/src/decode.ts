    ...
    switch (encoded[0]) {
        ...
        case ValueType.Object: {
                ...
                setValue(newValue) {
                    batch(() => {
                        if (typeof newValue === 'number') {
                            ...
                            setLength(Array.isArray(newValue)
                                ? newValue.length
            ...
            if (initValue) {
                for (const [key, child] of Object.entries(data)) {
                    initValue[key as any] = child === -1
                        ? {type: ValueType.Getter, name: key}
                        : decode(child)
                }
            }
        ...
        case ValueType.Store: {
            ...
            if (!store) {
                ...
                } as DecodedValue<ValueType.Store>)
...
export function updateCollapsedValue(
    ...
    const head = (
        list[data.type === ValueType.Store ? 1 : 0] as EncodedValue<
