...
const paddingValue = spacing[3.5]
const remMinusPx = `calc(${paddingValue} - 0.95px)`
...
export const levelPadding = style({
  ...
  width: `calc(${levelVar} * ${paddingValue} + ${spacing[2.5]})`,
  height: `calc(${rowHeight} + 0.95px)`,
  vars: {
    [linesColor]: color.gray[200],
  },
  background: `repeating-linear-gradient(to right, transparent, transparent ${remMinusPx}, ${linesColor} ${remMinusPx}, ${linesColor} ${paddingValue})`,
